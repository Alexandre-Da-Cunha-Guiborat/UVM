...
namespace UVM.Engine
{
    ...
    public static class UVMManager
    {
        ...
        public static List<I_VersionnableFile> ComputeDependencyTree(
            ...
            List<I_VersionnableFile> modifiedFiles)
        {
            ...
            List<I_VersionnableFile> filesToUpdateRec = _ComputeDependencyTreeRecursive(vfPool, modifiedFiles, 0, _maxIter);
            if (filesToUpdateRec is null)
            {
            ...
            List<I_VersionnableFile> filesToUpdate = modifiedFiles.Concat(filesToUpdateRec)
                                                                               .Distinct()
                                                                               .ToList();
        ...
        private static List<I_VersionnableFile> _ComputeDependencyTreeRecursive(
            List<I_VersionnableFile> vfPool,
            ...
            uint maxIter)
        {
            ...
            foreach (I_VersionnableFile depToCheck in seeds)
            {
                foreach (I_VersionnableFile vf in vfPool)
                {
                    foreach (I_VersionnableFile vfDep in vf.VFDeps)
                    {
            ...
            if (filesToUpdate.Any())
            {
                ...
                return _ComputeDependencyTreeRecursive(vfPool, seeds, nbIter, maxIter);
            }
            else
            {
                return seeds;
            ...
            uint maxIter)
        {
            ...
            foreach (I_VersionnableFile vfLeaf in vfLeafs)
            {
                foreach (I_VersionnableFile vf in vfPool)
                {
                    foreach (I_VersionnableFile vfDep in vfLeaf.VFDeps)
                    {
