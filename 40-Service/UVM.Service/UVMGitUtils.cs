    ...
    public static class UVMGitUtils
    {
        ...
        public static Boolean IsRebaseNeeded(String gitDirPath, String branchName, String commitIdRef, String commitId)
        {
            ...
            if (currentBranch is null)
            {
                ...
                UVMLogger.AddLog(ADCGLogLevelType.ERROR, titleNoCurrentBranch, messageNoCurrentBranch);
            ...
            if (refCommit is null || commit is null)
            {
                ...
                UVMLogger.AddLog(ADCGLogLevelType.ERROR, titleCommitNull, messageCommitNull);
            ...
            else
            {
                ...
                if (commonAncestor is null)
                {
                    ...
                    UVMLogger.AddLog(ADCGLogLevelType.INFO, titleAncestor, messageAncestor);
                ...
                UVMLogger.AddLog(ADCGLogLevelType.INFO, title, message);
            ...
            List<String> fExtensions)
        {
            ...
            if (modifiedFilesExts.Any())
            {
                foreach (I_VersionableFile vf in vfPool)
                {
                    ...
                    if (vfFileInfo.DirectoryName is null)
                    {
                        ...
                        UVMLogger.AddLog(ADCGLogLevelType.ERROR, errTitle, errLog);

                        return [];
                    }


                    String vfPath = Path.GetFullPath(vf.VFPath).Replace("\\", "/");
                    String parentDir = Path.GetFullPath(vfFileInfo.DirectoryName).Replace("\\", "/");
                    if (modifiedFilesExts.Where(mFExt => mFExt.Contains(parentDir) && !mFExt.Equals(vfPath)).Any())
                    {
                        vfToUpdate.Add(vf);
                    }
                }
            ...
            UVMLogger.AddLogListVF(ADCGLogLevelType.INFO, title, preface, vfToUpdate);
            ...
            List<String> commitIds)
        {
            ...
            List<String> modifiedFiles = GetGitDiffs(gitDirPaths, commitIdRefs, commitIds);
            List<String> modifiedFilesDirectory = modifiedFiles.Select(filePath => new FileInfo(filePath).DirectoryName?.Replace("\\", "/") ?? string.Empty).ToList();
            List<I_VersionableFile> vfAheadOfGit = vfPool.Where(vf => modifiedFilesDirectory.Where(mfDir => mfDir.Equals(vf.VFDirPath)).Any()).ToList();
            ...
            UVMLogger.AddLogListVF(ADCGLogLevelType.INFO, title, preface, vfAheadOfGit);
            ...
            List<String> fExtensions)
        {
            ...
            UVMLogger.AddLogListVF(ADCGLogLevelType.INFO, title, preface, vfAllKind);
        ...
        private static List<String> GetGitDiffCommitWithCommitRef(String gitDirPath, String commitIdRef, String commitId)
        {
            if (IsGitDirectory(gitDirPath) is false)
            {
                ...
                UVMLogger.AddLog(ADCGLogLevelType.INFO, titleNotGitRepo, messageNotGitRepo);
            ...
            if (changes is not null)
            {
                ...
                UVMLogger.AddLogList(ADCGLogLevelType.TRACE, title, preface, modifiedFiles);

                return modifiedFiles.Select(path => Path.GetFullPath(path).Replace("\\", "/")).ToList();
            }
            ...
            UVMLogger.AddLog(ADCGLogLevelType.INFO, titleNoChange, message);
